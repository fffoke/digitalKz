...
> **Флоу ролей (важно).** При регистрации НИКАКАЯ роль не выбирается — все
> получают базовую роль `user` и сразу попадают в приложение (лента, профиль,
> директ доступны всем). Роль ученика/преподавателя выбирается ПОЗЖЕ во вкладке
> «Обучение». Заявка **не блокирует**: человек продолжает пользоваться
> приложением, а о выдаче роли узнаёт из **пуш-уведомления**.
>
> `user` → (выбор в «Обучение» + модерация) → `student` | `teacher`

**`/register` — регистрация (без выбора роли)**
- `POST /api/auth/register` — { name, email, password } → создаётся `Role.user`
- после успеха → сразу в приложение (`/home`), НЕ на верификацию

**`/login`**
- `POST /api/auth/login` → JWT
- `GET /api/auth/me` — текущий пользователь (на каждом старте app)

**📖 Обучение → «Выберите свою роль»** (показывается, пока `role == user`)
- `GET /api/me/role-status` — { role, verification_status, teacher_application_status }
  → решает, что рисовать: выбор роли / «на модерации» / дашборд
- кнопка «Стать учеником» → экран `/verify`
- кнопка «Стать преподавателем» → экран анкеты `/teacher/apply`

**`/verify` — верификация ученика (ИИН + удостоверение)**
- `POST /api/verification` — multipart { iin, doc_photo } → статус `pending`
- после отправки → **возврат в приложение** (не сидит на экране ожидания)

**`/teacher/apply` — анкета преподавателя**
- `POST /api/teacher/apply` — { education, experience, kazakh_level } → `pending`

**Пуш о новой роли**
- модератор одобряет в админке → бэк меняет роль и создаёт уведомление
  `role_granted` → `GET /api/notifications` → фронт (PWA) показывает пуш/тост.
  Вкладка «Обучение» переключается на дашборд ученика/преподавателя.
- MVP: фронт опрашивает `GET /api/notifications` / `GET /api/me/role-status`;
  позже — реальный Web Push через service worker (PWA уже подключён).
